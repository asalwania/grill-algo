 ...
 * runs the real count-then-bucket algorithm — the SAME `resultOf` the paper
 * sheet drains, never a second copy — and refuses to let the two disagree.
 */
describe('authored answers', () => {
  it.each([EXAMPLE, ...CHECKS])(
    'the algorithm confirms $nums / k=$k → $result',
    ({ nums, k, result }) => {
      expect(resultOf(nums, k)).toBe(result)
    },
  )
   ...
   *
   * This problem accepts any order, so an exact-string match against `result`
   * would also pass if the algorithm returned a conveniently-ordered but WRONG
   * set. This counts independently and asserts the set that came back really
   * is a valid top k: k distinct values, and nothing left out is more frequent
   * than anything taken — the same second check `paper.test.ts` runs.
   */
  ...
  it('renders the SAME answer it pins — display and verification cannot drift', () => {
    ...
    for (const row of rendered) {
      expect(resultOf(row.nums, row.target as number)).toBe(row.result)
    }
  })
})
