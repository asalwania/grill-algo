 ...
 *  stand-in is enough to re-run the real two-pass algorithm from a check row. */
 ...
 * runs the real prefix/suffix algorithm — the SAME `resultOf` the paper sheet
 * drains, never a second copy — and refuses to let the two disagree.
 */
describe('authored answers', () => {
  it.each([EXAMPLE, ...CHECKS])(
    'the algorithm confirms $nums → $result',
    ({ nums, result }) => {
      expect(resultOf(asCase(nums))).toBe(result)
    },
  )

  it('agrees with an independent brute-force nested-loop product', () => {
    for (const c of [EXAMPLE, ...CHECKS]) {
      const expected = c.nums.map((_, i) =>
        c.nums.reduce((product, value, j) => (j === i ? product : product * value), 1),
      )
      const expectedStr = expected.length === 0 ? '[]' : `[${expected.join(', ')}]`
      expect(resultOf(asCase(c.nums))).toBe(expectedStr)
    }
  })
