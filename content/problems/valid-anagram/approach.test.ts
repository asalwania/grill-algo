...
/** `resultOf` takes a `PaperCase` — it only ever reads `.nums`/`.target`, so a
 *  minimal stand-in is enough to re-run the real algorithm from a check row. */
function asCase(nums: number[], target?: number): PaperCase {
  return { nums, target, expected: '', tag: '', reasoning: '' }
}

/** Unpacks the char-code + boundary packing `toCheck` produces, the same way
 *  `paper.ts`'s own `split` does, for the independent brute-force check below. */
function unpack(nums: number[], target?: number): { s: string; t: string } {
 ...
 * runs the real two-pass algorithm — the SAME `resultOf` the paper sheet drains,
 * never a second copy — and refuses to let the two disagree.
 */
describe('authored answers', () => {
  it.each([EXAMPLE, ...CHECKS])(
    'the algorithm confirms "$s" vs "$t" → $result',
    ({ s, t, result }) => {
      const nums = [...s, ...t].map((ch) => ch.charCodeAt(0))
      expect(resultOf(asCase(nums, s.length))).toBe(result)
    },
  )

  it('agrees with an independent count-based check', () => {
    for (const c of [EXAMPLE, ...CHECKS]) {
      ...
      if (c.s.length !== c.t.length) {
        ...
        for (const ch of c.s) counts.set(ch, (counts.get(ch) ?? 0) + 1)
        for (const ch of c.t) {
      ...
      const nums = [...c.s, ...c.t].map((ch) => ch.charCodeAt(0))
      expect(resultOf(asCase(nums, c.s.length))).toBe(expected)
    }
  })
  ...
  it('renders the SAME answer it pins — display and verification cannot drift', () => {
    ...
    for (const row of rendered) {
      expect(resultOf(asCase(row.nums, row.target))).toBe(row.result)
      // And the packing itself must round-trip, so `input`/`nums`/`target` can
      // never quietly drift apart from one another.
      const { s, t } = unpack(row.nums, row.target)
      expect(row.input).toBe(`"${s}" vs "${t}"`)
    }
  })
})
