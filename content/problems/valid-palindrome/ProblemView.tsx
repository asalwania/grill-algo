...
import { VALID_PALINDROME_CHROME } from "./chrome";

/**
 * Valid Palindrome's learning view — a thin wrapper that supplies this
 * problem's chrome and brief to the shared one. Everything else on the page
 * is common to the whole array-plus-memory family.
 *
 * A CLIENT module because `ProblemChrome` holds functions, which cannot
 * cross the RSC boundary; the route passes only plain data and lets this
 * supply the rest.
 */
export function ProblemView(
  ...
  return (
    <ArrayMemoryProblemView
      {...props}
      chrome={VALID_PALINDROME_CHROME}
      brief={ProblemBrief}
    />
  );
}
