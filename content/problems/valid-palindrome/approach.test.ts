 ...
 *
 * Every worked answer in the walkthrough is hand-authored — deliberately,
 * the same exception the paper sheet makes (lib/types.ts). This runs the
 * real two-pointer walk — the SAME `resultOf` the paper sheet drains, never
 * a second copy — and refuses to let the two disagree.
 */
describe('authored answers', () => {
  it.each([EXAMPLE, ...CHECKS])('the algorithm confirms $s → $result', ({ s, result }) => {
    expect(resultOf(s)).toBe(result)
  })

  it('agrees with an independent clean-and-reverse check', () => {
    for (const c of [EXAMPLE, ...CHECKS]) {
      const cleaned = [...c.s]
        .filter((ch) => /[a-z0-9]/i.test(ch))
        .map((ch) => ch.toLowerCase())
      const expected = cleaned.join('') === cleaned.slice().reverse().join('')
      expect(resultOf(c.s)).toBe(String(expected))
    }
  })
  ...
  it('renders the SAME answer it pins — display and verification cannot drift', () => {
    ...
    for (const row of rendered) {
      const s = row.nums.map((code) => String.fromCharCode(code)).join('')
      expect(resultOf(s)).toBe(row.result)
    }
  })
})

describe('the walkthrough shape', () => {
  it('is the nine-move sequence, in order', () => {
